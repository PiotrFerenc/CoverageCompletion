...
public class WorktreeManagerTests : IDisposable
{
    ...
    public async Task CreateAsync_CreatesWorktreeCheckedOutOnNewCoverageBranch()
    {
        ...
        _session.RepoPath.ShouldBe(_repo.Path);
        _session.BranchName.ShouldStartWith("coverage/session-");
        _session.BaseBranch.ShouldBe("main");
        Directory.Exists(_session.WorktreePath).ShouldBeTrue();
        File.Exists(Path.Combine(_session.WorktreePath, "README.md")).ShouldBeTrue();

        var checkedOutBranch = GitCli.Run(_session.WorktreePath, "rev-parse", "--abbrev-ref", "HEAD");
        checkedOutBranch.ShouldBe(_session.BranchName);
    }
    ...
    public async Task RemoveAsync_DeletesTheWorktreeDirectory()
    {
        ...
        Directory.Exists(_session.WorktreePath).ShouldBeFalse();
